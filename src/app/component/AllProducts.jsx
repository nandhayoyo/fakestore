...
const AllProducts = () => {
  const { products, setProducts, selectedCategory, searchResults } =
    useProductsStore();

  const [isLoading, setIsLoading] = useState(true); // State untuk mengendalikan apakah data sedang dimuat
  ...
  return (
    <section className="max-w-screen-xl mx-auto mt-5 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 p-4">
      {isLoading ? (
        [0, 1, 2, 3].map((index) => (
          <div
            key={index}
          ...
          >
            <Link href={`/product/${value.id}`}>
              ...
              <div className="flex flex-col justify-between px-5 pb-5 leading-normal mt-10">
                ...
                <span className="text-xl sm:text-2xl font-bold text-green-500 dark:text-white">
                  ${value.price}
                </span>
